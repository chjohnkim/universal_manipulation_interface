...
#                port=self.port) as wsg:
            with GripperDriver() as wsg:               
                # home gripper to initialize
                wsg.calibrate_gripper()
                # get initial
                curr_info = wsg.get_state()
                ...
                while keep_running:
                    ...
                    info = wsg.set_gripper_width(target_pos, wait=False)
                    #info = wsg.get_state()
                    #info['force'] = 0
                    #info['velocity'] = 0
                    #info = {
                    ...
                    state = {
                        'gripper_position': info['position'] / self.scale, # mm to m
                        'gripper_velocity': 0, #info['velocity'],# / self.scale,
                        'gripper_force': 0, #info['force'],
                        'gripper_receive_timestamp': time.time(),
                    ...
                    for i in range(n_cmd):
                        ...
                        if cmd == Command.SHUTDOWN.value:
                            ...
                            target_pos = command['target_pos'] * self.scale  # meters to mm

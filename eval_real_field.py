...
from image_cls.model import SimpleCNN
...
DEFAULT_HOME_POSE = np.array([[0.020881509408391428, -0.6444590687751599, -0.3021398186683829, 1.530478715896674, -0.008886680007001867, -0.19410791993136145]])
DEFAULT_PLACE_POSE1 = np.array([[0.03511899337173382, -0.6057280898093824, -0.20596443116668028, 1.5603089332581281, -0.038494117558044466, -0.15131917595860264]])
DEFAULT_PLACE_POSE2 = np.array([[-0.4472419917583389, -0.36577284336086274, -0.12453259527683361, 1.2520480155944893, -1.2637068033218593, -1.1297913789749385]])
...
def cls_grasp(img, cls_model, cls_transform):
    img = img[0]
    # Convert to integer
    img = (img * 255).astype(np.uint8)
    #img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    image = Image.fromarray(img).convert("RGB")
    image_tensor = cls_transform(image).unsqueeze(0).cuda()
    # Perform inference
    with torch.no_grad():
...
def main(input, output, robot_config, 
    ...
    with SharedMemoryManager() as shm_manager:
        with Spacemouse(shm_manager=shm_manager) as sm, \
            ...
            # Load grasp detector
            cls_model = SimpleCNN().cuda()
            cls_transform = transforms.Compose([
                transforms.Resize((128, 128)),
                transforms.ToTensor(),
            ])
            cls_model.load_state_dict(torch.load('./image_cls/best_model.pth'))
            ...
            while True:
                ...
                try:
                    ...
                    while True:
                        ...
                        # run classifier
                        if not execute_place:
                            img = deepcopy(obs['camera0_rgb'])
                            grasp = cls_grasp(img, cls_model, cls_transform)
                            if grasp == 'grasped':
                                grasp_count += 1
                            else:
                                grasp_count = 0
                        ...
                        if execute_place=='stage1':
                            ...
                            gripper_pos = 1
                            current_pose = np.array(np.stack([rs['TargetTCPPose'] for rs in env.get_robot_state()]))
                            if np.allclose(current_pose, DEFAULT_PLACE_POSE1, rtol=1e-2, atol=1e-2):
                                target_pose = np.copy(DEFAULT_PLACE_POSE2)
                                gripper_pos = 1
                                execute_place = 'stage2'
                        # Stage 2: Go to place position and release gripper. Go to waypoint 1
                        if execute_place=='stage2':
                            ...
                            gripper_pos = 1
                            current_pose = np.array(np.stack([rs['TargetTCPPose'] for rs in env.get_robot_state()]))
                            if np.allclose(current_pose, DEFAULT_PLACE_POSE2, rtol=1e-2, atol=1e-2):                    
                                target_pose = np.copy(DEFAULT_HOME_POSE)
                                gripper_pos = 0
                                execute_place = 'stage3'
                        # Stage 3: Go to home position
...
#                                target_pose = np.copy(DEFAULT_HOME_POSE)
#                                gripper_pos = 0
#                                execute_place = 'stage4'
#                        if execute_place=='stage4':
#                            target_pose = np.copy(DEFAULT_HOME_POSE)
#                            gripper_pos = 0
                        print(execute_place)
                        if execute_place:
                            this_target_poses = np.zeros_like(this_target_poses)
                            this_target_poses[:, :6] = target_pose[:6]
                            this_target_poses[:, 6] = gripper_pos
